...
parser.add_option('-T', '--maxticket', default=100, help='maximum ticket value, if randomly assigned', action='store',
...
parser.add_option('-d', '--diff', help='log difference between job endings', action='store_true', default=False,
                  dest='diff')
...
if len(joblist) == 2 and options.diff:
    endTimeList = []
elif options.diff == True and len(joblist) != 2:
    raise ValueError("Available only for 2 processes.")
        ...
        if jobs == 0:
            if options.diff:
                with open("stat", "a") as f:
                    f.write(str(abs(endTimeList[1] - endTimeList[0])) + " ")
                # Fairness metrics as in book Job1FinishTime/Job2FinishTime
                # f.write(str(abs(endTimeList[0]/endTimeList[1]))+" ")
            print('')
            break
